...
class DB {
  ...
  static public function get_events() {
    global $wpdb;
    return $wpdb->get_results( "SELECT id, title, lon, lat, info
                                FROM `{$wpdb->prefix}eventmap_data`;");
  }

  static public function get_event_by_id( $id ) {
    global $wpdb;
    return $wpdb->get_row( "SELECT id, title, lon, lat, info
                            FROM {$wpdb->prefix}eventmap_data
                            WHERE id = {$id};");
  }

  static public function insert_event( $title, $lat, $lon, $info = "" ) {
    global $wpdb;
    $sql = "INSERT INTO `{$wpdb->prefix}eventmap_data`(title, lat, lon, info)
            VALUES ('{$title}', {$lat}, {$lon}, '{$info}');";
    return $wpdb->query( $sql );

  }

  static public function update_event( $id, $title, $lat, $lon, $info = "" ) {
    global $wpdb;
